...
from laplacian import sample_pcd, paint_points
import numpy as np
...
if __name__ == "__main__":
    ...
    pcd_orig_d = pcd_orig.voxel_down_sample(voxel_size=0.03)
    #downm = m_c.voxel_down_sample(voxel_size=0.08)


    
    pcd_sampled = sample_pcd(pcd_orig_d, 
                             filter_type, 
                             n_samples, 
                             scale_min_dist, 
                             scale_max_dist,
    ...
    # o3d.io.write_point_cloud('s.ply',pcd_sampled)

    # pcd_orig.paint_uniform_color([0, 1, 0])

    # pcd_sampled.paint_uniform_color([1, 0, 0])

    # draw([pcd_orig, pcd_sampled])
    ...
    # # ------------------------------区域生长---------------------------------------
    # rg = reg.RegionGrowing(pcd_sampled,
    ...
    #     ind = indices[i]
    #     clusters_cloud =pcd_sampled.select_by_index(ind)
...
'''    # m_temp.colors=o3d.utility.Vector3dVector(m_match)
    ...
    downu = pcd_u.voxel_down_sample(voxel_size=0.01)
    downm = pcd_orig.voxel_down_sample(voxel_size=0.01)
