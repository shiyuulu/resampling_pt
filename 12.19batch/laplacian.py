...
def laplacian_smoothing(pcd_np, L,A, features, alpha=0.001, num_iter=500):
    """
...
#         x = x.reshape(N, 3)
#         Sx = S @ x 
#         laplacian_term =  gamma * np.sum((L.dot(Sx))**2)  
#         # np.sum((L.dot(Sx))**2)   
#         # np.sum(Sx.T @ L @ Sx)
...
#         # regularization_term = (gamma)*np.sum((L.dot(x))**2) 
#         uniform_term=( 2*np.linalg.norm(NS @ (x - pcd_np))**2 +np.sum((x - pcd_np)**2))
# # np.linalg.norm(S @ (x - pcd_np))**2 +

#         # np.sum((x - pcd_np)**2) 
        
#         # (np.linalg.norm(S @ (x - pcd_np))**2 +np.sum((x - pcd_np)**2))
#         # +np.sum((x - pcd_np)**2)

#         # uniform_term= (1-gamma)*np.sum((A.dot(Psi) - alpha * k)**2)

#         return laplacian_term  + uniform_term
#     # + regularization_term

#     def gradient(x):
#         x = x.reshape(N, 3)
        
#         laplacian_grad =  gamma * 2 *L.T.dot(L.dot(S@x))
#         # *L.T.dot(L.dot(x))
#         # L.dot(S@x)
...
#         # regularization_grad =(gamma) *2 *L.T.dot(L.dot(x)) 
#         uniformity_grad = (2*NS @ (x - pcd_np)+(x - pcd_np) )
#         # S @ (x - pcd_np)+
#         # (x - pcd_np)     
#         # (S @ (x - pcd_np)+(x - pcd_np) )
...
#         return (laplacian_grad+ uniformity_grad).flatten()
#     #  + regularization_grad
#     # + uniformity_term
    ...
    def objective(x):
        ...
        #np.sum(x.T@L@ x)            # #total vari
        # np.sum((L.dot(x))**2)       #smooth
        regularization_term = (1-gamma)*(2*np.linalg.norm(NS @ (x - pcd_np))**2+np.sum((x - pcd_np)**2))
    ...
    def gradient(x):
        ...
        # L.dot(x)                     #total vari
        # *L.T.dot(L.dot(x))            #smooth
        regularization_grad =(1-gamma) *2 *(4*NS @ (x - pcd_np)+ (x - pcd_np))
...
def compute_scores_from_points(pcd, filter_type, scale_min_dist, scale_max_dist, max_neighbor, var):
    ...
    # scores_pt=optimize_point_cloud(pcd_np, 20, F, D, W,  alpha=0.5, lambda_feature=0.1, lambda_uniform=0.5, gamma_smooth=0.1)
    ...
    o3d.io.write_point_cloud('s.ply',pcd_af)
